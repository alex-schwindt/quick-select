...
export default function App() {
  ...
  async function importCatalogFile() {
    if (!catalogFile) {
      setCatalogImportError('Please choose a catalog CSV or workbook first.');
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const issueText = Array.isArray(data?.issues) && data.issues.length ? ` ${data.issues.join(' | ')}` : '';
        throw new Error((data?.error || 'Catalog import failed.') + issueText);
      }
      ...
      <main className="workspace multi-layout">
        <section className="panel selector-panel">
          ...
          <div className="panel-header" style={{ marginTop: 24 }}>
            <div>
              <h2>Admin catalog import</h2>
              <p>Upload the normalized catalog template or a schedule-style workbook to refresh database matches.</p>
            </div>
          </div>

          <label className="form-field">
            <span className="meta-label">Catalog file</span>
            <input
              type="file"
              accept=".csv,.xlsx,.xls"
              onChange={(event) => {
          ...
          <div className="builder-actions">
            <button
              ...
              type="button"
              onClick={importCatalogFile}
              disabled={!catalogFile || isImportingCatalog}
            >
              {isImportingCatalog ? 'Importing catalog...' : 'Import catalog'}
            </button>
          </div>

          {catalogFile ? <p className="helper-text">Selected catalog file: {catalogFile.name}</p> : null}
          {catalogImportError ? <div className="error-banner">{catalogImportError}</div> : null}
          {catalogImportResult ? (
            <div className="logic-card">
              <span className="meta-label">Catalog import result</span>
              <ul>
                <li>Rows read: {catalogImportResult.rows_read ?? 0}</li>
                <li>Inserted: {catalogImportResult.inserted ?? 0}</li>
                <li>Updated: {catalogImportResult.updated ?? 0}</li>
              </ul>
              {Array.isArray(catalogImportResult.issues) && catalogImportResult.issues.length > 0 ? (
                <>
                  <p className="helper-text">Validation issues:</p>
                  <ul>
                    {catalogImportResult.issues.map((issue, index) => (
                      <li key={`catalog-issue-${index}`}>{issue}</li>
                    ))}
                  </ul>
                </>
              ) : (
                <p className="helper-text">Catalog import completed without validation issues.</p>
              )}
            </div>
          ) : null}
